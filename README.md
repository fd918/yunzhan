...
正式服务器构建时设置 `NEXT_PUBLIC_SERVER_AUTH=true`，由服务端统一拦截页面与接口访问；`scripts/build-server-release.mjs` 会在本地发布包副本中移除所有知识 HTML 对 `public/access-gate.js` 的引用，飞书登录后打开文档不再重复输入共享密码。静态兼容版继续保留该脚本，同一标签页验证一次后不重复提示；源码只保存密码校验摘要，不记录明文密码。

正式知识库可以内嵌到 `https://data.iyunzhanle.cn` 的外部看板区域。知识库响应头只允许自身和该正式数据看板作为父页面，不向其他网站开放；数据看板也只把 `https://zsk.iyunzhanle.cn` 加入 iframe 来源白名单。未登录用户在内嵌区域点击“在新窗口登录飞书”，授权完成后关闭窗口并回到看板点击“重新加载”；会话有效期间不再重复登录，也不会出现静态备份的共享密码门禁。
...
当前知识来源覆盖 7 个飞书群、112,605 条去重消息，消息范围为 2023-10-20—2026-08-23。2026-08-24 周度运行抓取 1,021 条消息，与既有基线按消息 ID 去重后新增 980 条；其中自动发现并首次全量补抓云瞻平台运营小分队 513 条：

| 群 | 去重消息数 | 主要沉淀方向 |
| --- | ---: | --- |
| 商务&客服业务沟通群 | 18,403 | 业务规则、活动口径、链接密令、数据与结算 |
| 客服问题反馈处理群 | 48,756 | 问题案例、排查步骤、处理结论与升级 |
| 客服中心内部沟通群 | 13,331 | 岗位制度、值班交接、审批与培训 |
| 客服问题反馈处理群-通知群 | 19,877 | 历史工单与周度新增工单的分类、优先级、状态与时效 |
| 需求讨论群 | 10,385 | 产品诉求、排期讨论、上线反馈和历史决策，按需求状态判断是否已上线 |
| 云瞻核心群 | 1,340 | 经营决策、跨部门统一口径、重点业务风险和上线确认 |
| 云瞻平台运营小分队 | 513 | 日报周报、早会、对外宣发复核、运营流程和 AI 工具安全 |
...
知识库内嵌策略由 `server/security.mjs` 统一维护。修改允许来源时必须同步检查数据看板的 `frame-src`，并用正式响应头确认知识库已移除 `X-Frame-Options: SAMEORIGIN`、`frame-ancestors` 仍只包含受信任父站点；不得使用 `*` 或绕过目标站点身份验证。
...
日常纯内容更新已于2026-09-08启用HTTPS内容接口，不再需要远程桌面或重启服务；认证边界、批次幂等、验收及回退见 [内容接口发布](docs/CONTENT_PUBLISHING.md)。每周任务已切换，原下方构建/程序发版说明只适用于程序或新页面变更。会话仍为12小时有效期，失效时需重新登录，不能宣称永久无人值守。
