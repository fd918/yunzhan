...
左侧“知识门户”提供“待确认知识”入口。正式服务器使用飞书 OAuth 登录和 PostgreSQL 持久化，不建立独立账号密码；本企业飞书账号第一次登录会自动成为“查看者”，管理员只需为需要维护知识的成员设置“编辑者”或“管理员”。跨企业账号会被拒绝，管理员手动停用的账号不会在再次登录时自动恢复。
...
| 管理员 | 拥有编辑能力，并可批准发布、驳回候选和管理账号角色与启用状态 |
...
4. 编辑者保存后进入“修改中”，管理员可以驳回；
5. 管理员点击“确认并发布”后记录审核人和时间，内容即时进入所选知识页的“审核确认补充”章节并加入服务端搜索；
6. 浏览器不持有飞书应用密钥、数据库密码或 GitHub 密钥，所有写入与发布动作都由服务端校验会话和角色。

正式迁移位于 `database/001_knowledge_workbench.sql`，回滚位于 `database/001_knowledge_workbench.rollback.sql`：
...
迁移只新增上述五张表、约束和索引，不修改“云瞻数据看板”的数据库表或现有知识页。正式回滚前先用 `pg_dump` 备份 `yunzhan_knowledge` 数据库，再停止 `YunzhanKnowledgeWeb` 任务、恢复 Nginx 备份并执行回滚脚本；GitHub Pages 静态备份不受影响。
...
2026-08-18 已登记上述回调并完成首位管理员初始化。后续本企业新账号首次登录自动获得查看权限；管理员进入“待确认知识 → 账号与权限”，通过角色下拉框将成员提升为编辑者或管理员，选择后立即保存。账号确需禁止访问时再点击“停用”；不再重复使用首次管理员令牌。
